...
const ViewTaoDNTTNhanh = ({navigation, route}) => {
    const dataKH = route.params.listSelectDaNhan;
    ...
    const getToken = async () => {
      ...
      useEffect(() => {
        setNoiDungDNTT(`${dataKH[0].TEN_CONG_TY} - ${dataKH[0].SO_CHUNG_TU} - ${noiDung}`);
        setDienGiai(`${dataKH[0].TEN_CONG_TY} - ${dataKH[0].SO_CHUNG_TU} - ${noiDung}`);
      }, [noiDung]);
      const buttonCreate = () => {
        dispatch(
          PostDeNghiTT(
...
<View style={styles.container}>
      ...
      <View style={styles.flex}>
          <Text style={styles.title}>
              {`${dataKH[0].TEN_CONG_TY} - ${dataKH[0].SO_CHUNG_TU}`}
          </Text>
      </View>
          <View style={styles.flex}>
...
export default ViewTaoDNTTNhanh;
